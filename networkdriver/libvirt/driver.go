...
func InitDriver(job *engine.Job) engine.Status {
	...
	libvirtNetwork := job.Getenv("BridgeIface")
	if libvirtNetwork != "" {
		var err error
		if bridgeIface, err = getBridgeIface(libvirtNetwork); err != nil {
			job.Error(fmt.Errorf("libvirt network %s doesn't exist: %s", err))
	...
	if err != nil {
		if bridgeIface != defaultBridgeName {
			job.Error(fmt.Errorf("libvirt network %s's bridge %s doesn't exist", libvirtNetwork, bridgeIface))
			return engine.StatusErr
		}
...
func createBridge(bridgeIP string) error {
	...
	if err := createBridgeIface(ifaceAddr); err != nil {
		return err
	}
...
func createBridgeIface(address string) error {
