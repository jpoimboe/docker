...
func (container *Container) Start() (err error) {
	...
	if err := mount.Mount(container.socketPath(), path.Join(root, "/.dockersocket"), "none", "bind,ro"); err != nil {
		return err
	}
...
func (container *Container) dockerInitRpcCall(method string, args, reply interface{}) error {
	...
	if err := container.dockerInitRpc.Call("DockerInitRpc."+method, args, reply); err != nil {
...
func (container *Container) connectToDockerInit(reconnect bool) error {
	...
	if err != nil {
		return fmt.Errorf("socket connection failed: %s", err)
	}
...
func (container *Container) socketPath() string {
	return path.Join(container.root, "socket")
}
